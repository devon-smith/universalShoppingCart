...
The background-refresh worker (Phase 7, `POST /api/refresh`) reads it server-side through
...
## Background refresh (Phase 7)

The worker is `POST /api/refresh` on the web app (Node runtime), authorised by a `CRON_SECRET`
bearer and driven by Supabase Cron. One-time setup in the production project:

1. Dashboard → Database → Extensions: enable `pg_cron` and `pg_net`.
2. In Vercel, set `SUPABASE_SERVICE_ROLE_KEY` and `CRON_SECRET` (server-only, Sensitive — never a
   `NEXT_PUBLIC_` prefix). The route reads both.
3. In the Supabase SQL editor, store the worker URL and the **same** cron secret in Vault, then
   schedule the job (every 30 minutes here):

```sql
select vault.create_secret('https://YOUR-APP.example/api/refresh', 'refresh_worker_url');
select vault.create_secret('YOUR-CRON-SECRET', 'refresh_cron_secret'); -- == Vercel CRON_SECRET

select cron.schedule('universal-cart-refresh', '*/30 * * * *', $$
  select net.http_post(
    url := (select decrypted_secret from vault.decrypted_secrets where name = 'refresh_worker_url'),
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer ' || (select decrypted_secret from vault.decrypted_secrets where name = 'refresh_cron_secret')
    ),
    body := '{}'::jsonb
  );
$$);
...
The worker claims due `public_fetch` jobs, fetches each under the SSRF rules, records an
observation and any alerts, and reports outcomes for backoff. To test before enrollment is
automatic, enqueue an item by hand: `select public.enqueue_refresh_job('<item-id>');` then either
wait for the schedule or `POST /api/refresh` with the bearer. Automatic enrollment from the save
path is a follow-up.
