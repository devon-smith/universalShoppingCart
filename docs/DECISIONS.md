...
## 2026-08-03 — AI comparison summary runs server-side through the Claude API

**Decision.** Phase 8's fact-grounded comparison summary is generated by the Claude API
(`@anthropic-ai/sdk`, model `claude-opus-5`), called only from the web server — a Server Action
(`features/compare/summary-action.ts`) behind an explicit button. A new major dependency, hence
this ADR. Cross-retailer identifier matching, the other half of §22's Phase 8, stays cut (see the
2026-07-26 clothing/comparison entries) — clothing shoppers compare distinct candidates, so there
is no "same product at two retailers" to match.

**Context.** BUILD_PLAN.md §16.2 sets the guardrails: AI runs server-side only, the provider key
never enters a client bundle, only the smallest product metadata (never page HTML) is sent, model
and prompt version are stored as provenance, and AI never overwrites a deterministic field. The
compare view already computes a structured, grounded `Comparison` from stored item fields, so the
model is fed _that_ — the same facts the table shows — never raw HTML, cookies, or notes-as-input.
The prompt names every missing field so the summary reports gaps instead of inventing values.

**Consequences.** The key is read from `AI_PROVIDER_API_KEY` (no `NEXT_PUBLIC_`/`WXT_PUBLIC_`
prefix, so Next.js cannot inline it) via a `server-only` module; an unset key degrades to a plain
"not enabled" state, not an error. Prompt-building, the output schema, and the orchestrator's
guardrails (schema validation, refusal handling, rejecting a fabricated item reference) are pure
and unit-tested with an injected model call — no live key needed for CI. Summaries are stored in
`comparison_summaries` with model + prompt-version provenance; the row doubles as a cache keyed on
a hash of the grounded facts (which folds in the prompt version, so a prompt change regenerates).
The cache is cart-scoped under RLS, so a cross-cart comparison is summarised but **not** stored —
caching it could expose one cart's facts to a reader of the other. The live model call is gated on
a human setting `AI_PROVIDER_API_KEY` in Vercel (server-only, Sensitive); until then the feature is
