-- Fibre content, persisted.
--
-- docs/DECISIONS.md (2026-08-02): composition becomes a real field now that comparison has
-- a consumer for it. It is stored raw — "100% cotton", "Shell: 100% wool; Lining: 52%
-- polyester" — with no normalization, and it describes the garment, so it is a
-- retailer-observed column: the reject_observed_field_writes trigger protects it and the
-- ingestion function writes it, exactly like price or availability. It is emphatically not
-- in selected_variant, because that feeds the fingerprint and a spec table that renders on
-- one visit and not the next would then hash one product two ways (the 2026-07-27 entry).

alter table public.items
  add column composition text;

comment on column public.items.composition is
  'Fibre content as the page published it, raw and un-normalized — "100% cotton", '
  '"Shell: 100% wool; Lining: 52% polyester". A retailer-observed field; a client cannot '
  'write it. Null when the page said nothing about composition.';

-- A retailer-observed column, so a client update must leave it untouched.
...
-- Re-declare the ingestion function. Identical to 20260731090000 except for reading and
-- writing composition, marked below.
...
begin
  ...
  -- Raw fibre content, as the page published it. Null when the page said nothing.
  v_composition := nullif(btrim(p_capture -> 'product' ->> 'composition'), '');
  ...
  if v_existing.id is null then
    insert into public.items (
      ...
      title, brand, description, composition, image_url,
    ...
    set
      ...
      -- Composition barely changes, so a capture that did not read it keeps the stored
      -- value rather than wiping it — coalesce, like brand and description.
      composition = coalesce(v_composition, composition),
      image_url = coalesce(
        nullif(btrim(p_capture -> 'product' ->> 'selectedImageUrl'), ''),
        image_url
      ),
